...
export async function GET(req: NextRequest, res: NextResponse) {
  if (req.method === 'GET') {
    ...
    try {
      ...
      if(!listType || !address){
        return new NextResponse(JSON.stringify({ success: false, error: '!listType || !address'}), {
                  status: 500,
                  headers: {
      ...
      const tokensM = await Tokens(listType, address);
...
async function Tokens(listType: string, address: string) {
  ...
  const token = tokensList.filter((t: any) => t.address.toLowerCase().includes(address.toLowerCase()))
